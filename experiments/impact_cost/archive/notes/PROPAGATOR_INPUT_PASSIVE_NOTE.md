...
The canonical lifecycle experiment uses this formulation directly as its fixed
tail-propagator impact model. Lower-level impact helpers still accept
`propagator_tail` as a compatibility alias. The implementation evaluates the
path at consuming market-event times, mirroring the old Rust single-queue
helper:
