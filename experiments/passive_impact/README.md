...
Knobs: `time_horizon`, `n_simulations`, `initial_queue_size`, `mode` (`"single"` | `"double"`), `side` (`"with"` | `"without"` | `"both"`), the Hawkes parameters `mu`/`alpha`/`beta`, the affine-queue parameters `a_l`/`b_l`/`a_c`/`b_c`, and the metaorder shape (either an integer count for evenly-spaced orders inside `metaorder_window`, or an explicit `list`/`np.ndarray` of arrival times).
