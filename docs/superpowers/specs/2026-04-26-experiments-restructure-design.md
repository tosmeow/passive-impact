...
├── load_experiments/
│   ├── data/                       # .npy files (gitignored; regenerated by Rust binaries)
...
The pre-existing `*.npy` rule in `.gitignore` is preserved: `.npy` files are not committed and live as working-tree artefacts under each `load_experiments/data/` subtree. To populate a fresh checkout, run the Rust binaries (Section 6). Notebooks force-add (`git add -f`) where needed to ship the analysis layer with the repo.
