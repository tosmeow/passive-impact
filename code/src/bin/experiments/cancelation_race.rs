...
fn main() {
    ...
    println!(
        "Conditioning cancels: {} events in [0, {}]",
        n_cancels, burst_window_end
    );
    println!(
        "External limits: {} events in [0, {}] (for queue stability)",
        n_limits, burst_window_end
    );
    println!(
        "Burst: {} cancels at time x with spacing {}",
        p_burst, epsilon
    );
    println!(
        "Testing {} different x values in (0, {})",
        n_x_values, burst_window_end
    );
    ...
    println!(
        "Generated {} conditioning cancel events",
        conditioning_cancels.events.len()
    );
    ...
    println!(
        "Generated {} external limit events",
        external_limits.events.len()
    );
    ...
    println!(
        "[TIMING] q simulation (reference): {:?} ({} total events)",
        t0.elapsed(),
        q_path.events.len()
    );
    ...
    let sim_configs: Vec<SimulationConfig<Vec<f64>>> = configs
        .iter()
        .map(|(_x, externals)| {
            SimulationConfig::new(Some(externals), Some(bar_initial_state.clone()))
        })
        .collect();
    ...
    for sim_result in sim_results.iter() {
        ...
        let queue_diff: Vec<i32> = bar_q_samples_grid
            .iter()
    ...
    write_npy_f64_1d(
        &format!("{}/conditioning_cancel_times.npy", output_dir),
        &conditioning_times,
    )
    .unwrap();
    ...
    // Write initial_delta
    write_npy_f64_1d(
        &format!("{}/initial_delta.npy", output_dir),
        &[initial_delta as f64],
    )
    .unwrap();
    ...
    println!(
        "  - queue_paths_grid.npy: [{} x {}] queues at sample grid",
        n_sample_times,
        n_sims + 1
    );
    println!(
        "  - queue_diff_grid.npy: [{} x {}] queue differences (bar_q - q) at sample grid",
        n_sample_times, n_sims
    );
    ...
    println!(
        "  - conditioning_cancel_times.npy: [{}] conditioning cancel times",
        n_cancels
    );
}
