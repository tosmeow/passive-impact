...
pub struct ConditionalQueueSimulator<'a> {
    lambda_l: Box<dyn Fn(f64) -> f64>,
    lambda_c: Box<dyn Fn(f64) -> f64>,
    time_horizon: f64,

    // Borrowed slices - no cloning needed
    limit_orders: &'a [f64],
    cancel_orders: &'a [f64],
    market_orders: &'a [f64],
    meta_orders: &'a [f64],

    // Outputs remain owned
...
impl<'a> ConditionalQueueSimulator<'a> {
    pub fn new(
    ...
    pub fn simulation(
        ...
        while t < self.time_horizon {
            ...
            let tau_l_bar = if lambda_l_bar == 0.0 {f64::INFINITY} else {sample_exponential(&mut rng, lambda_l_bar)};
            ...
            let tau_c_bar = if lambda_c_bar == 0.0 {f64::INFINITY} else {sample_exponential(&mut rng, lambda_c_bar)};

            // Find minimum tau
            let taus = [tau_l_bar, tau_c_bar, tau_n - t, tau_c - t, tau_l - t, tau_m - t];
